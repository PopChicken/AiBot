...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,vip.popop.pcr.gGHelper";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("vip.popop.pcr.gGHelper", 1, 9, "爱梅斯群助手", "1.0.0", 1, "Polstar", "爱梅斯群助手", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("vip.popop.pcr.gGHelper", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("vip.popop.pcr.gGHelper", AppData.CQLog);	
